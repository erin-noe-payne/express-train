...
var LIFECYCLE_STAGES = ['config', 'load', 'start'];
...
function registerLifecycleCallbacks(libDir){
    ...
    _.each(events, function(libs, event){
        if(!_.contains(LIFECYCLE_STAGES, event)) {
            throw new Error('Unrecognized lifecycle stage '+event);
        }
        if(_.isString(libs)) {
            listen(lib, event);
        }
            ...
            throw new Error('Bad index.js. Event handlers should be a string or array referencing lib files.');
        }
    });
